...
class ReloadMenu: UIView {
    ...
    override func didMoveToSuperview() {
        ...
        buttonAcept.addTarget(self, action: "reload", forControlEvents: UIControlEvents.TouchUpInside)
        ...
        buttonReject.addTarget(self, action: "play", forControlEvents: UIControlEvents.TouchUpInside)
        ...
        if let font =  UIFont(name: "Skranji-Bold", size: GLOBALbuttonSize.height * 0.45 ) {
            ...
            let myMutableString = NSMutableAttributedString(string: NSLocalizedString("AskReloadGame", comment: ""), attributes: textFontAttributes)
    ...
    func reload() {
        ...
        if let menuGame = menu as? Panel {
            ...
            (menuGame.game as! GameScene).loadGame((menuGame.game as! GameScene).currentLevel)
        }
    }
    func hide() {
        if GLOBALsounds {
